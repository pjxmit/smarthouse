...
static int app_bin_len = 0;

static esp_err_t search_content_len(const char *buffer, int *content_length){
	char key[] = "Content-Length:";
	char *pstr = strstr((char*)buffer, key);

	printf("%s", buffer);

	if(pstr) {
		*content_length = atoi(pstr+strlen(key));
		printf("%s: %d\n", key, *content_length);
		return ESP_OK;
    }
    return ESP_FAIL;
...
static bool erase_http_header(uint8_t *buff, int total_len, int *data_len)
{
	...
	search_content_len((const char *)buff, &app_bin_len);
...
static esp_err_t ota_download(int socket_id, esp_ota_handle_t ota_handle, SSL *ssl)
{
    ...
    for (;;) {
		...
		if(ssl)
			recv_len = SSL_read(ssl, data_buff, BUFF_SIZE);
		else
        	recv_len = recv(socket_id, data_buff, BUFF_SIZE, 0);
		//printf("%d\n", recv_len);
        if (recv_len < 0) {
            return ESP_FAIL;
        }
		...
		if(total_len >= app_bin_len && app_bin_len && resp_start)
			break;
    }
    return ESP_OK; 
}

esp_err_t iot_ota_start(const char *server_ip, uint16_t server_port, const char *file_dir, uint32_t ticks_to_wait, int ssl_flag)
{
	...
	if(ssl)
		ret = SSL_write(ssl, http_request, get_len);
	else
    ...
    ret = ota_download(socket_id, upgrade_handle, ssl);
...
esp_err_t iot_ota_start_url(const char *url, uint32_t ticks_to_wait)
{
    ...
    ret = iot_ota_start(server_addr, port, file_path, ticks_to_wait, ssl_flag);
