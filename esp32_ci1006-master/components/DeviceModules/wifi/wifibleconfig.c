...
esp_err_t BleConfigStart(uint32_t ticks_to_wait)
{
    ...
    if (ret) {
        ESP_LOGE(WIFI_BLE_TAG, "%s Bluetooth controller release classic bt memory failed", __func__);
        return ESP_FAIL;
    }
	...
	inBleconfig = true;
//    esp_event_loop_init(audio_ble_net_event_handler, NULL);
    EventBits_t uxBits;
	if(ticks_to_wait == 0) {
